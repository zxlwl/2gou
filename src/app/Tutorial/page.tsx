...
export default function Page() {
	...
	useEffect(() => {
		const needPassword = siteContent.enablePasswordAccess && 
			...
			(siteContent.passwordAccessCategories || []).includes('Tutorial')
		
		if (needPassword) {
			// 检查是否已经验证过，并且密码没有更改
			const verified = localStorage.getItem('password_Tutorial') === 'verified'
			const storedHash = localStorage.getItem('password_Tutorial_hash')
	...
	// 筛选出分类为"分享教程"的文章
	const tutorialArticles = articles.filter(item => item.category === '分享教程')
	
	// 获取所有标签
	const allTags = Array.from(new Set(tutorialArticles.flatMap(article => article.tags || [])))
	
	// 根据搜索词和标签筛选文章
	const filteredArticles = tutorialArticles.filter(article => {
	...
	console.log('tutorialArticles length:', tutorialArticles.length)
	console.log('filteredArticles length:', filteredArticles.length)
	console.log('allTags:', allTags)

	if (!isVerified) {
		return <PasswordVerify category="Tutorial" onVerify={() => setIsVerified(true)} />
	}
